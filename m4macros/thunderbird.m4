...
[
  ...
  THUNDERBIRD_ADITIONAL_INCLUDE_PATH="$THUNDERBIRD_BUILT_DIR/mozilla/dist/include/mozilla"

  case "$host_os" in
    mingw*|cygwin*)
      THUNDERBIRD_INCLUDE_PATH=`cygpath -w $THUNDERBIRD_INCLUDE_PATH`
      THUNDERBIRD_ADITIONAL_INCLUDE_PATH=`cygpath -w $THUNDERBIRD_ADITIONAL_INCLUDE_PATH`
  ...
  MJ_CHECK_MOZILLA_VERSION
  AC_SUBST(XPCOM_LIBS)
  AC_SUBST(XPCOM_LDFLAGS)
  AC_SUBST(XPCOM_CFLAGS)

  THUNDERBIRD_INCLUDE_PATH="$THUNDERBIRD_INCLUDE_PATH;$THUNDERBIRD_ADITIONAL_INCLUDE_PATH"
...
AC_DEFUN([MJ_CHECK_MOZILLA_VERSION],
[
